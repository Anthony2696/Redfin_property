...
def search(entry,driver,search_bar):
    try:
        move_to_element(driver,search_bar)
        try:
            search_bar.clear()
            search_bar.clear()
            driver.find_element_by_xpath(f'//input[@title="Clear"]').click()
        except:
            pass
        time.sleep(1)
#         driver.execute_script(f"document.getElementById('search-box-input').getElementsByTagName('textarea')[0].value={entry}")
...
#         search_bar.send_keys(Keys.RETURN)
        ...
        print("Exception in search function: ",f)
        return False
...
def get_response(url,driver):
    ...
    try:
        ...
        response = TextResponse(url,body=r.content,encoding='utf-8')
        #soup = BeautifulSoup(r.content,"lxml")
        return response
    except Exception as e:
        print("Exception in Get Response",e)
        #driver.find_element_by_xpath('//input[@title="Clear"][@class="btn-clear-search-input clickable"]')
        return ''
